...
import { ShoppingBag, Briefcase, Shield, Sparkles } from 'lucide-react';

export default function Login() {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [selectedRole, setSelectedRole] = useState('buyer'); // 'buyer', 'seller', 'admin'
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to login. Please verify credentials.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center pt-24 pb-12 px-4 bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold font-display text-gray-900 dark:text-white">Account Login</h2>
          <p className="text-xs mt-2 text-gray-400 dark:text-gray-500 uppercase tracking-widest font-semibold">Sign in to govern, trade or explore</p>
        </div>

        {/* Dynamic Category Selector for Login */}
        <div className="mb-6 bg-gray-50 dark:bg-gray-850 p-1.5 rounded-2xl border border-gray-150 dark:border-gray-800 flex gap-2">
          <button 
            type="button"
            onClick={() => { setSelectedRole('buyer'); setEmail(''); setPassword(''); }}
            className={`flex-1 py-3 text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2 ${
              selectedRole === 'buyer' 
          ...
          <button 
            type="button"
            onClick={() => { setSelectedRole('seller'); setEmail(''); setPassword(''); }}
            className={`flex-1 py-3 text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2 ${
              selectedRole === 'seller' 
                ? 'bg-black text-white dark:bg-white dark:text-black shadow-md' 
                : 'text-gray-500 hover:text-black dark:hover:text-white'
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div>
            <label className="block text-[10px] font-bold tracking-widest uppercase text-gray-500 dark:text-gray-400 mb-2">Password</label>
            <input
              type="password"
              value={password}
          ...
          <button 
            ...
            disabled={loading}
          >
            {loading ? 'SIGNING IN...' : `LOG IN AS ${selectedRole.toUpperCase()}`}
          </button>
        </form>

        <p className="mt-8 text-center text-xs text-gray-400">
          Don't have an account?{' '}
          <Link to="/signup" className="text-black dark:text-white hover:underline font-bold transition-colors">
            Create an account
          </Link>
        </p>
      </motion.div>
